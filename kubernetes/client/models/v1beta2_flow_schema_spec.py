...
class V1beta2FlowSchemaSpec(BaseModel):
    """
    FlowSchemaSpec describes how the FlowSchema's specification looks like.
    """ # noqa: E501
    distinguisher_method: Optional[V1beta2FlowDistinguisherMethod] = Field(default=None, alias="distinguisherMethod")
    ...
    priority_level_configuration: V1beta2PriorityLevelConfigurationReference = Field(alias="priorityLevelConfiguration")
    rules: Optional[List[V1beta2PolicyRulesWithSubjects]] = Field(default=None, description="`rules` describes which requests will match this flow schema. This FlowSchema matches a request if and only if at least one member of rules matches the request. if it is an empty slice, there will be no requests matching the FlowSchema.")
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "distinguisherMethod": V1beta2FlowDistinguisherMethod.from_dict(obj.get("distinguisherMethod")) if obj.get("distinguisherMethod") is not None else None,
            "matchingPrecedence": obj.get("matchingPrecedence"),
            "priorityLevelConfiguration": V1beta2PriorityLevelConfigurationReference.from_dict(obj.get("priorityLevelConfiguration")) if obj.get("priorityLevelConfiguration") is not None else None,
            "rules": [V1beta2PolicyRulesWithSubjects.from_dict(_item) for _item in obj.get("rules")] if obj.get("rules") is not None else None
        })
        return _obj
